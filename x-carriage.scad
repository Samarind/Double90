...
module x_carriage_assembled() {
    ...
    *for (sign = [-1, 1]) {
    ...
    for (sign = [-1, 1]) {
        translate([sign * (X_bearings_holder_length / 2  - 2 * wall), 0, axle_height]) {
            translate([0, -bearing_outer_diameter(X_bearings) / 2 - wall + 8.3, 0])
                rotate([90, 0, 0]) 
                    screw(M4_pan_screw, 15, center = true);
            translate([0, -(-bearing_outer_diameter(X_bearings) / 2 - wall + 5), 0])
                rotate([90, 0, 0]) 
                    nut(M4_nut);
        }
        ...
        difference() {
            union() {
                ...
                translate([0, 0, x_bar_spacing()])
                    rotate([90, 0, 90]) 
                        cylinder(r = bearing_outer_diameter(X_bearings) / 2 + wall, h = X_bearings_holder_length, $fn = smooth, center = true);

                // Main plate
                translate([0, 0, x_bar_spacing() / 2 + wall / 2 - wall])
                    cube([X_bearings_holder_length, bearing_outer_diameter(X_bearings) + 2 * wall, x_bar_spacing() + 2 * wall], center = true);

            }

            //Front rail cutout
            translate([0, -bearing_outer_diameter(X_bearings) / 2 - wall + 4, x_bar_spacing() / 2])
                rotate([0, 90, 0]) 
            ...
            for (sign = [-1, 1]) {
                translate([sign * (X_bearings_holder_length / 2  - 2 * wall), 0, axle_height]) {
                    ...
                    translate([0, -bearing_outer_diameter(X_bearings) / 2 - wall + 6.1, 0])
                        rotate([90, 0, 0]) 
