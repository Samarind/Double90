...
//bar_clamp(Z_smooth_rod_diameter, gantry_setback, bar_clamp_depth, true);
...
module z_bar_clamp_stl()        translate([0,0,bar_clamp_depth/2]) rotate([0,90,0]) bar_clamp(Z_smooth_rod_diameter, gantry_setback, bar_clamp_depth, false, false);
module z_bar_clamp_switch_stl() translate([0,0,bar_clamp_depth/2]) rotate([0,90,0]) bar_clamp(Z_smooth_rod_diameter, gantry_setback, bar_clamp_depth, true, false);

module bar_clamps_stl() {
	y2 = bar_clamp_length(Z_smooth_rod_diameter) - bar_clamp_tab + 2;
	y3 = y2 + bar_clamp_length(Y_bar_dia) - bar_clamp_tab + 2;
																						 rotate([0, 0, 180]) z_bar_clamp_switch_stl();
	translate([2, -2 * bar_rail_offset(Z_smooth_rod_diameter) + bar_clamp_length(Z_smooth_rod_diameter), 0])                         z_bar_clamp_stl();
...
if(0)
	...
	z_bar_clamp_assembly(Z_smooth_rod_diameter, gantry_setback, bar_clamp_depth, true);
	//bar_clamp(Z_smooth_rod_diameter, gantry_setback, bar_clamp_depth, true, false);
