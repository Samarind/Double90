...
module x_end_bracket() {
        ...
        difference() {
            union() {
                // Main vertical block
                translate([-clamp_length / 2 - 1, X_smooth_rod_diameter / 2 + Z_bearing_holder_width / 2, (x_bar_spacing() + clamp_width) / 2]) {
                    difference() {
                        cube([clamp_length, clamp_width, x_bar_spacing()], center = true);
                        cube([clamp_length + eta, X_smooth_rod_diameter - wall, x_bar_spacing() - 1.5 * clamp_width], center = true); 
                        translate([0, 0, (x_bar_spacing() - 1.5 * clamp_width) / 2])
                            rotate([90, 0, 90])  
                                cylinder(h = clamp_length + eta, r = (X_smooth_rod_diameter - wall) / 2, $fn = smooth, center = true);  
                    }
                    ...
                    translate([wall, 0, 0])
                        difference() {
                            ...
                            translate([0, 0, -(x_bar_spacing() / 3 + wall) / 2])
                                rotate([90, 0, 90])  
                                    cylinder(h = slit * 2 + eta, r = X_smooth_rod_diameter / 2 - wall / 2, $fn = smooth, center = true);
                        }
                }
            ...
            if (is_hex(Z_nut)) {
                ...
                translate([-z_bar_spacing(), 0, nut_depth(Z_nut) / 2 + wall]) 
                    poly_cylinder(r = nut_outer_radius(Z_nut), h = nut_depth(Z_nut) + 0.1, $fn = smooth, center = true);
                translate([-z_bar_spacing() - nut_outer_radius(Z_nut) / 2 - wall, 0, nut_depth(Z_nut) / 2 + wall])
                    cube([nut_outer_radius(Z_nut) + 2 * wall, nut_outer_radius(Z_nut) * 2 + 0.2, nut_depth(Z_nut) + 0.1], center = true);
            }

            // Hole for spectra line bearing screw
            translate([-clamp_length * 3 / 4 - 1, Z_bearing_holder_width / 2 - 1.5 * wall, (x_bar_spacing() + clamp_width) / 2])
                rotate([90, 90, 0])
                    nut_trap(M4_clearance_radius, M4_nut_radius, M4_nut_trap_depth, true, length = M4_nut_trap_depth);

            // Hole for spectra line fixing screw
            translate([-clamp_length / 4 - 1, clamp_width / 2 + Z_bearing_holder_width / 2 + wall, (x_bar_spacing() + clamp_width) / 2])
                rotate([90, 90, 0])
...
module x_end_assembly() {
    ...
    *if (is_hex(Z_nut)) {
    ...
    translate([-clamp_length * 3 / 4 - 1, X_smooth_rod_diameter / 2 + Z_bearing_holder_width / 2, (x_bar_spacing() + clamp_width) / 2])
        rotate([0, 90, 90]) 
            ball_bearing(BB624);

    // Spectra line bearing screw
    translate([-clamp_length * 3 / 4 - 1, clamp_width + Z_bearing_holder_width / 2 - wall, (x_bar_spacing() + clamp_width) / 2])
        rotate([0, 90, 90]) 
            screw_and_washer(M4_pan_screw, screw_longer_than(clamp_width), center = true);

    // Spectra line fixing screw
    *translate([-clamp_length / 4 - 1, clamp_width + Z_bearing_holder_width / 2 - wall, (x_bar_spacing() + clamp_width) / 2])
        rotate([0, 90, 90])
